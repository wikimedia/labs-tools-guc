    ...
    <body>
        <div class="maincontent">
            ...
            <form method="POST" class="searchField" id="searchForm">
                <p> IP Address or username: <input name="user" value="<?php if($guc){print(htmlspecialchars($guc->getUsername()));}elseif(key_exists('user', $_GET)){ print(htmlspecialchars($_GET['user'])); } ?>" class="usernameOrIp"></p>
            ...
            if($guc) {
                ...
                if($guc->getAlloverCount() > 0) print($guc->getAlloverCount().' edits found in '.$guc->wikisWithEditscount.' projects.');
                print('</p>');
                print('<div class="results">');
                if($guc->getHostname()) print('<div class="hostname">'.$guc->getUsername().' = '.$guc->getHostname().'</div>');
                foreach ($guc->getData() as $wiki) {
                    if(is_object($wiki['data'])){
                        ...
                        if($wiki['url']) print('<h1>'.htmlspecialchars ($wiki['url']).'</h1>');
                        print($wiki['error']);
                        print('</div>');
                    }
