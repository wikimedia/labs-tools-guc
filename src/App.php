...
class App {
	...
	public function debug( $text ) {
		$this->times[] = [ microtime( true ), $text ];
	}
	...
	public function wikiparser( $comment, $page, $server ) {
		...
		$comment = preg_replace_callback(
			'!(?:(?<=(.)))?/\*\s*(.*?)\s*\*/(?:(?=(.)))?!',
			static function ( $match ) use ( $page, $server, &$append ) {
				$match += [ '', '', '', '' ];
				$isPre = $match[1] !== '';
				$auto = $match[2];
				$isPost = $match[3] !== '';
	...
	public function getNamespaceName( $id, $dbName, $server ) {
		...
		$apiData = $this->apiRequest( $server, [ 'meta' => 'siteinfo', 'siprop' => 'namespaces' ] );
